...
class UserModal extends Component{
    ...
    handleSubmit(e) {
        e.preventDefault();
        const { fullname, email, selectedGroups } = this.state;
        if(!fullname){
            ...
            const groups = selectedGroups.map(o => o.value);
            this.setState({loading: true}, function(){
                if(this.state.type === "add"){
                    this.props.addUser({fullname, email, groups});
                }else{
                    this.props.updateUser(this.state.usermodal._id, {fullname, email, groups});
                }
            })
        }
    }
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if(props.usermodal && props.usermodal !== state.usermodal){
            const groups = props.usermodal.groups;
            return{
                usermodal: props.usermodal,
                fullname: props.usermodal.fullname,
                email: props.usermodal.email,
                groups: groups,
                selectedGroups: groups && groups.map(o => ({value: o._id, label: o.title})),
    ...
    render(){
        const groups = this.props.groups.map(o => ({value: o._id, label: o.title}));
        return(
